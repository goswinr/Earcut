...
[<AllowNullLiteral>]
type Node (i: int, x: float, y: float) =
    ...
    member val prev: Node = null with get, set
    /// next vertex nodes in a polygon ring
    member val next: Node = null with get, set
    /// z-order curve value
    member val z: int = 0 with get, set
    /// previous nodes in z-order
    member val prevZ: Node = null with get, set
    /// next nodes in z-order
    member val nextZ: Node = null with get, set
    /// indicates whether this is a Steiner point
    member val steiner: bool = false with get, set
...
let inline internal notNull (node: Node) : bool = not<| isNull node
...
let inline internal signedArea (data: ResizeArray<float>, start: int, end': int, dim: int) : float =
    let mutable sum = 0.0
    let mutable i = start
    let mutable j = end' - dim
    while i < end' do
...
let inline internal linkedList (data: ResizeArray<float>, start: int, end': int, dim: int, clockwise: bool) : Node =
    let mutable last = null

    if clockwise = (signedArea(data, start, end', dim) > 0.0) then
        let mutable i = start
        while i < end' do
            last <- insertNode(i / dim, data.[i], data.[i + 1], last)
            i <- i + dim
    else
        let mutable i = end' - dim
        while i >= start do
            last <- insertNode(i / dim, data.[i], data.[i + 1], last)
            i <- i - dim
...
let inline internal filterPoints (start: Node, end': Node) : Node =
    if isNull start then
        start
    else
        let mutable end' = if isNull end' then start else end'

        let mutable p = start
        let mutable again = true // true initially so that the while loop runs at least once
        let mutable loopOn = true

        while loopOn && (again || p !== end') do
            again <- false

            if not p.steiner && (equals(p, p.next) || area(p.prev, p, p.next) = 0.0) then
                removeNode(p)
                end' <- p.prev      // in js p = end = p.prev
                p    <- end'      // in js p = end = p.prev
                if p === p.next then
                    loopOn <- false
                else
                    again <- true
...
let internal zOrder (x: float, y: float, minX: float, minY: float, invSize: float) : int =
...
let internal isEarHashed (ear: Node, minX: float, minY: float, invSize: float) : bool =
    ...
    if area(a, b, c) >= 0.0 then false // reflex, can't be an ear
        ...
        while notNull n && n.z <= maxZ && result do
...
let internal isValidDiagonal (a: Node, b: Node) : bool =
...
let internal sortLinked (list: Node) : Node =
    let mutable numMerges = 0
    let mutable inSize = 1
    let mutable resultList = list

    let mutable continueOuter = true
    while continueOuter do
        let mutable p = resultList
        let mutable e : Node = null
        resultList <- null
        let mutable tail : Node = null
        numMerges <- 0

        while not (isNull p) do
...
let inline internal indexCurve (start: Node, minX: float, minY: float, invSize: float) : unit =
    let mutable p = start
    let mutable first = true
    while first || p !== start do
        first <- false
...
let inline internal cureLocalIntersections (start: Node, triangles: ResizeArray<int>) : Node =
    let mutable start' = start //shadowing to make it mutable
    let mutable p = start
    let mutable loopOnce = true
    while p !== start'  || loopOnce do
        loopOnce <- false
        let a = p.prev
        let b = p.next.next
        ...
        while continue' do
            ...
            if equals(hole, p.next) then
                ...
                if x <= hx && x > qx then
                    ...
                    if x = hx then
                        continue' <- false // hole touches outer segment; pick leftmost endpoint
            if continue' then
                p <- p.next
                if p === outerNode then
                    continue' <- false
...
let inline internal eliminateHole (hole: Node, outerNode: Node) : Node =
...
let inline internal compareXYSlope (a: Node) (b: Node) : int =
...
let internal eliminateHoles (data: ResizeArray<float>, holeIndices: ResizeArray<int>, outerNode: Node, dim: int) : Node =
    ...
    while i < len do
        ...
        let end' = if i < len - 1 then holeIndices.[i + 1] * dim else data.Count
        let list = linkedList(data, start, end', dim, false)
        if list === list.next then list.steiner <- true
        queue.Add(getLeftmost(list))
        i <- i + 1

    queue.Sort(Comparison<Node>(compareXYSlope))
...
and internal earcutLinked (ear: Node, triangles: ResizeArray<int>, dim: int, minX: float, minY: float, invSize: float, pass: int) : unit =
    if notNull ear then
        ...
        while ear'.prev !== ear'.next && loopOn do
            let prev = ear'.prev
            let next = ear'.next

            if (if invSize <> 0.0 then isEarHashed(ear', minX, minY, invSize) else isEar(ear')) then
                triangles.Add(prev.i)
                triangles.Add(ear'.i)
                triangles.Add(next.i) // cut off the triangle
...
let deviation (data: ResizeArray<float>, holeIndices: ResizeArray<int>, dim: int, triangles: ResizeArray<int>) : float =
    ...
    if hasHoles then
        ...
        while i < len do
            let start = holeIndices.[i] * dim
            let end' = if i < len - 1 then holeIndices.[i + 1] * dim else data.Count
            polygonArea <- polygonArea - abs(signedArea(data, start, end', dim))
            i <- i + 1
...
let flatten (data: ResizeArray<ResizeArray<ResizeArray<float>>>) : {| vertices: ResizeArray<float>; holes: ResizeArray<int>; dimensions: int |} =
